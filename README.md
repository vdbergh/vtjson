...
- An object matches the schema `quote(schema)` if it is equal to `schema`. For example the schema `{"cats", "dogs"}` matches strings `"cats"` and `"dogs"` but the schema `freeze({"cats", "dogs"})` matches the the set `{"cats", "dogs"}`. 
