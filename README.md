...
- An object matches the schema `freeze(schema)` if it is equal to `schema`. For example the schema `str` matches strings but the schema `freeze(str)` only matches the object `str` (the string _type_ that is). 
